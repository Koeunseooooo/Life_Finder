...
class CalendarView(generic.ListView):
    ...
    def get_queryset(self, **kwargs):
        ...
        queryset = {
            'today_list_items': Event.objects.all().filter(profile=self.request.user.user_profile).filter(
                start_time__date=self.kwargs['date']),
            'today_list_rating_sum': Event.objects.all().filter(profile=self.request.user.user_profile).filter(
                start_time__date=self.kwargs['date']).aggregate(Sum('rating')).values(),
...
def dash(request):
    ...
    present_month_total = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__gte=present_month).aggregate(Sum('rating')).values()
    ...
    present_month_total_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__gte=present_month).count()
    ...
    pre_month_end_total = Event.objects.all().filter(profile=request.user.user_profile).filter(
    ...
    two_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=two_days_ago).aggregate(Sum('rating')).values()
    ...
    three_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=three_days_ago).aggregate(Sum('rating')).values()
    ...
    four_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=four_days_ago).aggregate(Sum('rating')).values()
    ...
    five_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=five_days_ago).aggregate(Sum('rating')).values()
    ...
    six_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=six_days_ago).aggregate(Sum('rating')).values()
    ...
    seven_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    eight_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=eight_days_ago).aggregate(Sum('rating')).values()
    ...
    nine_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=nine_days_ago).aggregate(Sum('rating')).values()
    ...
    ten_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=ten_days_ago).aggregate(Sum('rating')).values()
    ...
    eleven_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=eleven_days_ago).aggregate(Sum('rating')).values()
    ...
    twelve_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twelve_days_ago).aggregate(Sum('rating')).values()
    ...
    thirteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=thirteen_days_ago).aggregate(Sum('rating')).values()
    ...
    fourteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=fourteen_days_ago).aggregate(Sum('rating')).values()
    ...
    fifteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=fifteen_days_ago).aggregate(Sum('rating')).values()
    ...
    sixteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=sixteen_days_ago).aggregate(Sum('rating')).values()
    ...
    seventeen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=seventeen_days_ago).aggregate(Sum('rating')).values()
    ...
    eighteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=eighteen_days_ago).aggregate(Sum('rating')).values()
    ...
    nineteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=nineteen_days_ago).aggregate(Sum('rating')).values()
    ...
    twenty_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twenty_days_ago).aggregate(Sum('rating')).values()
    ...
    twentyone_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentyone_days_ago).aggregate(Sum('rating')).values()
    ...
    twentytwo_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentytwo_days_ago).aggregate(Sum('rating')).values()
    ...
    twentythree_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentythree_days_ago).aggregate(Sum('rating')).values()
    ...
    twentyfour_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentyfour_days_ago).aggregate(Sum('rating')).values()
    ...
    twentyfive_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentyfive_days_ago).aggregate(Sum('rating')).values()
    ...
    twentysix_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentysix_days_ago).aggregate(Sum('rating')).values()
    ...
    twentyseven_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentyseven_days_ago).aggregate(Sum('rating')).values()
    ...
    twentyeight_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentyeight_days_ago).aggregate(Sum('rating')).values()
    ...
    twentynine_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=twentynine_days_ago).aggregate(Sum('rating')).values()
    ...
    thirty_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=thirty_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_one_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_one_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_two_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_two_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_three_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_three_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_four_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_four_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_five_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_five_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_six_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_six_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_seven_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_seven_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_eight_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_eight_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_nine_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_nine_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_ten_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_ten_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_eleven_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_eleven_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twelve_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twelve_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_thirteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_thirteen_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_fourteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_fourteen_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_fifteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_fifteen_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_sixteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_sixteen_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_seventeen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_seventeen_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_eighteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_eighteen_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_nineteen_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_nine_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twenty_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twenty_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentyone_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentyone_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentytwo_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentytwo_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentythree_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentythree_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentyfour_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentyfour_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentyfive_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentyfive_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentysix_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentysix_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentyseven_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentyseven_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentyeight_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentyeight_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_twentynine_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_twentynine_days_ago).aggregate(Sum('rating')).values()
    ...
    pre_thirty_days_ago_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        start_time__date=pre_thirty_days_ago).aggregate(Sum('rating')).values()
    ...
    now = Event.objects.all().filter(profile=request.user.user_profile).filter(start_time__gte=one_month_ago).aggregate(
        Sum('rating')).values()
    for ing in now:
        ing
    ...
    # 운동 선택한 필드만 가져오는 쿼리문
    exercise_field = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='운동').filter(start_time__gte=seven_days_ago)
    # 운동 선택한 필드의 개수만 가져오는 쿼리문
    exercise_field_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='운동').filter(start_time__gte=seven_days_ago).count()
    exercise_field_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='운동').filter(start_time__gte=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    # 여행 선택한 필드만 가져오는 쿼리문
    travel_field = Event.objects.all().filter(profile=request.user.user_profile).filter(category__contains='여행').filter(
        start_time__gte=seven_days_ago)
    # 여행 선택한 필드의 개수만 가져오는 쿼리문
    travel_field_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='여행').filter(start_time__gte=seven_days_ago).count()
    travel_field_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='여행').filter(start_time__gte=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    # 기타 선택한 필드만 가져오는 쿼리문
    etc_field = Event.objects.all().filter(profile=request.user.user_profile).filter(category__contains='기타').filter(
        start_time__gte=seven_days_ago)
    # 기타 선택한 필드의 개수만 가져오는 쿼리문
    etc_field_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='기타').filter(start_time__gte=seven_days_ago).count()
    etc_field_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='기타').filter(start_time__gte=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    friend_field = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='친구/가족과의 시간').filter(start_time__gte=seven_days_ago)
    # 친구/가족과의 시간  선택한 필드의 개수만 가져오는 쿼리문
    friend_field_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='친구/가족과의 시간').filter(start_time__gte=seven_days_ago).count()
    friend_field_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='친구/가족과의 시간').filter(start_time__gte=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    self_field = Event.objects.all().filter(profile=request.user.user_profile).filter(category__contains='자기계발').filter(
        start_time__gte=seven_days_ago)
    # 자기계발 선택한 필드의 개수만 가져오는 쿼리문
    self_field_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='자기계발').filter(start_time__gte=seven_days_ago).count()
    self_field_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='자기계발').filter(start_time__gte=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    hobby_field = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='취미생활').filter(start_time__gte=seven_days_ago)
    # 취미생활 선택한 필드의 개수만 가져오는 쿼리문
    hobby_field_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='취미생활').filter(start_time__gte=seven_days_ago).count()
    hobby_field_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='취미생활').filter(start_time__gte=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    leisure_field = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='여가생활').filter(start_time__gte=seven_days_ago)
    # 여가생활 선택한 필드의 개수만 가져오는 쿼리문
    leisure_field_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='여가생활').filter(start_time__gte=seven_days_ago).count()
    leisure_field_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='여가생활').filter(start_time__gte=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    # 일 선택한 필드만 가져오는 쿼리문
    work_field = Event.objects.all().filter(profile=request.user.user_profile).filter(category__contains='일').filter(
        start_time__gte=seven_days_ago)
    # 일 선택한 필드의 개수만 가져오는 쿼리문
    work_field_count = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='일').filter(start_time__gte=seven_days_ago).count()
    work_field_rating = Event.objects.all().filter(profile=request.user.user_profile).filter(
        category__contains='일').filter(start_time__gte=seven_days_ago).aggregate(Sum('rating')).values()
    ...
    events = Event.objects.filter(start_time__gte=past_datetime).filter(profile=request.user.user_profile).filter(
        start_time__lte=future_datetime).order_by('start_time')

    # -6일째 라이프기록 객체들 불러올때
    one_select_datetime = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(days=-6).datetime
    one_select_datetime_late = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(
        days=-5).datetime
    one_select_events = Event.objects.filter(start_time__gte=one_select_datetime).filter(
        profile=request.user.user_profile).filter(start_time__lte=one_select_datetime_late).order_by('start_time')

    # -5일째 라이프기록 객체들 불러올 떄
    two_select_datetime = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(days=-5).datetime
    two_select_datetime_late = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(
        days=-4).datetime
    two_select_events = Event.objects.filter(start_time__gte=two_select_datetime).filter(
        profile=request.user.user_profile).filter(start_time__lte=two_select_datetime_late).order_by('start_time')

    # -4일째 라이프기록 객체들 불러올 떄
    three_select_datetime = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(days=-4).datetime
    three_select_datetime_late = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(
        days=-3).datetime
    three_select_events = Event.objects.filter(start_time__gte=three_select_datetime).filter(
        profile=request.user.user_profile).filter(start_time__lte=three_select_datetime_late).order_by('start_time')

    # -3일째 라이프기록 객체들 불러올 떄
    four_select_datetime = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(days=-3).datetime
    four_select_datetime_late = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(
        days=-2).datetime
    four_select_events = Event.objects.filter(start_time__gte=four_select_datetime).filter(
        profile=request.user.user_profile).filter(start_time__lte=four_select_datetime_late).order_by('start_time')

    # -2일째 라이프기록 객체들 불러올 때
    five_select_datetime = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(days=-2).datetime
    five_select_datetime_late = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(
        days=-1).datetime
    five_select_events = Event.objects.filter(start_time__gte=five_select_datetime).filter(
        profile=request.user.user_profile).filter(start_time__lte=five_select_datetime_late).order_by('start_time')

    # -1일째 라이프기록 객체들 불러올 때
    six_select_datetime = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(days=-1).datetime
    six_select_datetime_late = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(
        days=0).datetime
    six_select_events = Event.objects.filter(start_time__gte=six_select_datetime).filter(
        profile=request.user.user_profile).filter(start_time__lte=six_select_datetime_late).order_by('start_time')

    # today 라이프 기록 객체를 불러올 떄
    seven_select_datetime = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).datetime
    seven_select_datetime_late = arrow.utcnow().to('Asia/Seoul').replace(hour=0, minute=0, second=0).shift(
        days=1).datetime
    seven_select_events = Event.objects.filter(start_time__gte=seven_select_datetime).filter(
        profile=request.user.user_profile).filter(start_time__lte=seven_select_datetime_late).order_by('start_time')
    ...
    for b in list:
        if (max(list) == list[0]):
            mon_str = '월(MON)요병을 이겨내다니!😎'
        elif (max(list) == list[1]):
            tue_str = '화(TUE)창한 화요일😎'
        elif (max(list) == list[2]):
            wed_str = '이번주 수요일을 수(WED)월하게😎'
        elif (max(list) == list[3]):
            thur_str = '목요일은 목(THUR)표 달성하는 날!😎'
        elif (max(list) == list[4]):
            fri_str = '라이프로 채운 불.금(FRI)😎'
        elif (max(list) == list[5]):
            sat_str = '토요일 토(SAT)요일은 YAHO!😎'
        elif (max(list) == list[6]):
            sun_str = '일(SUN)상 가득!😎'
    ...
    context = {
        ...
        'a': a,
        # a는 뭐야? # 나도 몰라 은서가 넣은 거 아니였어? #웅?내가 말한건데??
...
def dash_detail(request):
    ...
    return render(request, 'cal/dash_detail.html', context=context)
